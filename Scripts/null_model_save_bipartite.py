    ...
    for networkName in indicators:
        ...
        if(independentNetworkExportPath.exists()):
            ...
            if(networkName=="usctextsimilarity"):
                ...
                # save bipartiteEdges to a pickle file
                with open(networksPath/f"{dataName}_{networkName}_bipartiteEdges.pkl", "wb") as f:
                    pickle.dump({"bipartiteEdges": bipartiteEdges,"dfFiltered":dfFiltered}, f, protocol=pickle.HIGHEST_PROTOCOL)
                # creates a null model output from the bipartite graph
