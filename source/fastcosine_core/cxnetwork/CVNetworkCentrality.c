...
CVBool CVNetworkCalculateCentrality_weighted_parallel_implementation(const CVNetwork* network,CVDoubleArray* centrality, CVOperationControl* operationControl){
	...
	CVDoubleArrayReallocToCapacity(verticesCount, centrality);
	CVDoubleArraySetCount(verticesCount, centrality);
		//memset(centrality->data, 0, verticesCount*sizeof(CVDouble));
	CVIndex i;
	...
	CVParallelForStart(centralityLoop, blockIndex, unrolledLoops){
		...
		for(s=blockIndex*unrolledSize;s<maxIndex;s++){
			...
			CVDouble sWeight = verticesWeights[s];
			S.count = 0;
			for (i=0; i<verticesCount; i++) {
				P[i].count=0;
				d[i] = -1.0;
			...
			while (fh_dequeue(Q,&vdata,&dist)) {
				...
				for (e=0; e<vEdgesCount; e++) {
					w = vNeighbor[e];
					double weight = edgesWeights[vEdge[e]];
...
CVBool CVNetworkCalculateCentrality_weighted_implementation(const CVNetwork* network,CVDoubleArray* centrality, CVOperationControl* operationControl){
	CVSize verticesCount = network->verticesCount;
...
#if CV_ENABLE_PARALLELISM
CVBool CVNetworkCalculateCentrality_parallel_implementation(const CVNetwork* network,CVDoubleArray* centrality, CVOperationControl* operationControl){
	CVSize verticesCount = network->verticesCount;
...
CVBool CVNetworkCalculateCentrality_implementation(const CVNetwork* network,CVDoubleArray* centrality, CVOperationControl* operationControl){
	CVSize verticesCount = network->verticesCount;
...
CVBool CVNetworkCalculateCentrality(const CVNetwork* network,CVDoubleArray* centrality, CVOperationControl* operationControl){
...
#if CV_ENABLE_PARALLELISM
CVBool CVNetworkCalculateStressCentrality_parallel_implementation(const CVNetwork* network,CVDoubleArray* centrality, CVOperationControl* operationControl){
	CVSize verticesCount = network->verticesCount;
...
CVBool CVNetworkCalculateStressCentrality_implementation(const CVNetwork* network,CVDoubleArray* centrality, CVOperationControl* operationControl){
	CVSize verticesCount = network->verticesCount;
...
CVBool CVNetworkCalculateStressCentrality(const CVNetwork* network,CVDoubleArray* centrality, CVOperationControl* operationControl){
